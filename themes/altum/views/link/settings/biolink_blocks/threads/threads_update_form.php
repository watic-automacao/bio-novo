...
<form name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <label for="<?= 'threads_location_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('biolink_threads.location_url') ?></label>
        <input id="<?= 'threads_location_url_' . $row->biolink_block_id ?>" type="text" class="form-control" name="location_url" value="<?= $row->location_url ?>" maxlength="2048" placeholder="<?= l('biolink_threads.location_url_placeholder') ?>" required="required" />
    </div>
