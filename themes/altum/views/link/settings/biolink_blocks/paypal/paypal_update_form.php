...
<form name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <label for="<?= 'paypal_type_' . $row->biolink_block_id ?>"><i class="fab fa-fw fa-paypal fa-sm text-muted mr-1"></i> <?= l('global.type') ?></label>
        <select id="<?= 'paypal_type_' . $row->biolink_block_id ?>" name="type" class="custom-select">
            <?php foreach(['buy_now', 'add_to_cart', 'donation'] as $key): ?>
                <option value="<?= $key ?>" <?= $row->settings->type == $key ? 'selected="selected"' : null ?>><?= l('biolink_paypal.type_' . $key) ?></option>
            <?php endforeach ?>
        </select>
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_email_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-envelope fa-sm text-muted mr-1"></i> <?= l('biolink_paypal.email') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'paypal_title_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-heading fa-sm text-muted mr-1"></i> <?= l('biolink_paypal.title') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'paypal_currency_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-euro-sign fa-sm text-muted mr-1"></i> <?= l('biolink_paypal.currency') ?></label>
        <input id="<?= 'paypal_currency_' . $row->biolink_block_id ?>" type="text" name="currency" maxlength="8" class="form-control" value="<?= $row->settings->currency ?>" placeholder="<?= l('biolink_paypal.currency_placeholder') ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'paypal_price_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-dollar-sign fa-sm text-muted mr-1"></i> <?= l('biolink_paypal.price') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'paypal_thank_you_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('biolink_paypal.thank_you_url') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'paypal_cancel_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('biolink_paypal.cancel_url') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'paypal_name_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('biolink_link.name') ?></label>
    ...
    <div class="form-group custom-control custom-switch">
        ...
        >
        <label class="custom-control-label" for="<?= 'paypal_open_in_new_tab_' . $row->biolink_block_id ?>"><?= l('biolink_link.open_in_new_tab') ?></label>
    </div>
    ...
    <div class="form-group" data-file-image-input-wrapper data-file-input-wrapper-size-limit="<?= settings()->links->thumbnail_image_size_limit ?>" data-file-input-wrapper-size-limit-error="<?= sprintf(l('global.error_message.file_size_limit'), settings()->links->thumbnail_image_size_limit) ?>">
        <label for="<?= 'link_image_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-image fa-sm text-muted mr-1"></i> <?= l('biolink_link.image') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'link_animation_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-film fa-sm text-muted mr-1"></i> <?= l('biolink_link.animation') ?></label>
    ...
    <div class="form-group" data-animation="<?= implode(',', require APP_PATH . 'includes/biolink_animations.php') ?>">
        <label for="<?= 'link_animation_runs_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-play-circle fa-sm text-muted mr-1"></i> <?= l('biolink_link.animation_runs') ?></label>
