...
<form name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="<?= 'map_address_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-map-marker-alt fa-sm text-muted mr-1"></i> <?= l('biolink_map.address') ?></label>
        <input id="<?= 'map_address_' . $row->biolink_block_id ?>" type="text" class="form-control" name="address" value="<?= $row->settings->address ?>" maxlength="64" placeholder="<?= l('biolink_map.address_placeholder') ?>" required="required" />
    </div>

    <div class="form-group">
        <label for="<?= 'map_markers_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paragraph fa-sm text-muted mr-1"></i> <?= l('biolink_map.markers') ?></label>
        <textarea id="<?= 'map_markers_' . $row->biolink_block_id ?>" class="form-control" name="markers" maxlength="1024"><?= $row->settings->markers ?></textarea>
        <small class="form-text text-muted"><?= l('biolink_map.markers_help') ?></small>
    </div>

    <div class="form-group">
        <label for="<?= 'map_location_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-link fa-sm text-muted mr-1"></i> <?= l('biolink_link.location_url') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'map_zoom_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-search fa-sm text-muted mr-1"></i> <?= l('biolink_map.zoom') ?></label>
    ...
    <div class="form-group">
        <label for="<?= 'map_type_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-map fa-sm text-muted mr-1"></i> <?= l('global.type') ?></label>
        <select id="<?= 'map_type_' . $row->biolink_block_id ?>" name="type" class="custom-select">
            <option value="roadmap" <?= $row->settings->type == 'roadmap' ? 'selected="selected"' : null ?>><?= l('biolink_map.type.roadmap') ?></option>
            <option value="satellite" <?= $row->settings->type == 'satellite' ? 'selected="selected"' : null ?>><?= l('biolink_map.type.satellite') ?></option>
            <option value="terrain" <?= $row->settings->type == 'terrain' ? 'selected="selected"' : null ?>><?= l('biolink_map.type.terrain') ?></option>
            <option value="hybrid" <?= $row->settings->type == 'hybrid' ? 'selected="selected"' : null ?>><?= l('biolink_map.type.hybrid') ?></option>
        </select>
    </div>
