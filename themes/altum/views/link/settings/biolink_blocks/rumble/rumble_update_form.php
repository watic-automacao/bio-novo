...
<form name="update_biolink_" method="post" role="form" data-type="<?= $row->type ?>">
    ...
    <div class="form-group">
        <label for="<?= 'rumble_location_url_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-video fa-sm text-muted mr-1"></i> <?= l('biolink_rumble.location_url') ?></label>
        <input id="<?= 'rumble_location_url_' . $row->biolink_block_id ?>" type="url" class="form-control" name="location_url" value="<?= $row->location_url ?>" placeholder="<?= l('biolink_rumble.location_url_placeholder') ?>" required="required" />
    </div>
